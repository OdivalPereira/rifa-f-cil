...
## 2024-05-23 - Gamification & Probability
**Learning:** Users love visual feedback for luck-based mechanics. Using canvas-confetti and CSS animations for a "Lucky Wheel" significantly enhances the perceived value of rewards. Implementing probabilities securely requires server-side logic (Edge Functions) to prevent client-side manipulation.
**Action:** For any luck-based feature, decouple the visual result (UI) from the actual determination (Backend). Use distinct animation states (Spinning -> Result -> Celebration) to build anticipation.
