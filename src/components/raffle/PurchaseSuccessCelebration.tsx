...
export function PurchaseSuccessCelebration({
  ...
  const { data: totalNumbers, isLoading: numbersLoading } =
    useUserTotalNumbers(buyerPhone);
  const { data: userAccount, isLoading: accountLoading } =
    useUserReferralCode(buyerPhone);
  ...
  const progressPercentage = Math.min(
    (totalQuantity / targetQuantity) * 100,
    100,
  );
    ...
    <Card className="w-full max-w-lg mx-auto card-jackpot border-gold/20 overflow-hidden relative">
      ...
      <CardHeader className="text-center space-y-2 pt-6 sm:pt-8 px-4 sm:px-6 relative">
        ...
        <p className="text-muted-foreground">
          ...
          <span className="text-gold font-bold">{purchasedQuantity}</span>{" "}
          número(s).
        </p>
      </CardHeader>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <Progress
                ...
                aria-label="Progresso para desbloquear link de indicação"
              />
              ...
              {isUnlocked ? (
                ...
                <div className="text-center space-y-4">
                  <p className="text-sm text-gray-300">
                    ...
                    números. Faltam apenas{" "}
                    <span className="text-gold font-bold">
                      {remainingQuantity}
                    </span>{" "}
                    para desbloquear seu Link da Sorte e concorrer à viagem
                    extra! ✈️
                  </p>
